...
// Arrays para las opciones originales e invertidas
const opcionesOriginales = [];
const opcionesInvertidas = [];

// Función para inicializar las opciones
function inicializarOpciones() {
    const opcionesDestinos = document.getElementById("destinos").options;
    opcionesOriginales.length = 0;
    opcionesInvertidas.length = 0;

    for (let i = 0; i < opcionesDestinos.length; i++) {
        const opcion = opcionesDestinos[i];
        opcionesOriginales.push(opcion);
        opcionesInvertidas.unshift(opcion);
    }
}
...
function actualizarOpciones() {
    ...
    // Vaciar el select de destinos
    selectDestinos.innerHTML = "";

    // Seleccionar las opciones según el grupo
    let opcionesMostrar = grupoSeleccionado === "ETON > Villa Regina" ? opcionesInvertidas : opcionesOriginales;

    opcionesMostrar.forEach((opcion) => {
        // Mostrar solo las opciones que no deben ocultarse
        if (
            (grupoSeleccionado === "Villa Regina > Neuquén" &&
                (opcion.value.includes("Villa Regina (Final del recorrido)") || opcion.value.includes("ETON (Inicio del recorrido)"))) ||
            (grupoSeleccionado === "ETON > Villa Regina" &&
                (opcion.value.includes("Villa Regina (Inicio del recorrido)") ||opcion.value.includes("ETON (Final del recorrido)")))
        ) {
